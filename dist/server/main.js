(() => {
  ...
  var getStringFromUInt8Array = (buffer, start, end) => String.fromCharCode(...buffer.slice(start, end)).replace(/\u0000/g, "");

  // node_modules/.pnpm/@nativewrappers+client@1.6.0/node_modules/@nativewrappers/client/lib/utils/getUInt32FromUint8Array.js
  var getUInt32FromUint8Array = (buffer, start, end) => new Uint32Array(buffer.slice(start, end).buffer)[0];
  ...
  var Vehicle = class extends Entity {
    constructor(handle) {
      super(handle);
    }
...
${word} `;
  ...
  // shared/utils.ts
  var WEAPON_LIST = /* @__PURE__ */ new Map();
  ...
  // server/main.ts
  RegisterCommand("resetmodel", (src) => {
    let plyState = Player(src).state;
    plyState.set("gunModel", 0, true);
  }, false);
  RegisterCommand("setweapon", (src, args) => {
    const hash = GetHashKey(args[0]);
    let plyState = Player(src).state;
    plyState.set("gunModel", hash, true);
  }, false);
  AddStateBagChangeHandler("gunModel", null, (bagName, key, value, reserved, replicated) => {
